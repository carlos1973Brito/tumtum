...
public interface ICalificacionesDao extends JpaRepository<Calificaciones,Long>{
	...
	@Query(value = "select * from t_calificaciones where id_t_usuarios=?1 and id_t_materias=?2", nativeQuery = true)
	List<Calificaciones> findByMateriaUsuario(Integer idMaterias,Integer idUsuario);
